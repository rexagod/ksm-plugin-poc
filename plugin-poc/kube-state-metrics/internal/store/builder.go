...
func (b *Builder) buildDeploymentStores() []cache.Store {
	deploymentPlugin, err := plugin.Open("/home/rexagod/plugin-poc/plugin-poc/kube-state-metrics/external/plugins/plugin.so")
	if err != nil {
		klog.Infoln(err)
	}
	symbolPlugin, err := deploymentPlugin.Lookup("Plugin")
	if err != nil {
		klog.Infoln(err)
	}
	type DPlugin interface {
		Generate([]string, []string) []generator.FamilyGenerator
	}
	dPlugin, ok := symbolPlugin.(DPlugin)
	if !ok {
		klog.Infoln("dPlugin symbol mismatch")
	}
	return b.buildStoresFunc(dPlugin.Generate(b.allowAnnotationsList["deployments"], b.allowLabelsList["deployments"]), &appsv1.Deployment{}, createDeploymentListWatch, b.useAPIServerCache)
}
